...
VL53L1_Error VL53L1Init(VL53L1_Dev_t* pDev,uint8_t Slave_Address)
{
	...
	VL53L1_SetDeviceAddress(pDev , Slave_Address);
	pDev->I2cDevAddr=Slave_Address;
...
VL53L1_Error VL53InitParam(VL53L1_Dev_t* pDev,uint8_t mode)
{
	...
	if(status!=VL53L1_ERROR_NONE) 
		return status;
	IIC_Delay_ms(2);
	status = VL53L1_SetInterMeasurementPeriodMilliSeconds(pDev, 100);
	if(status!=VL53L1_ERROR_NONE) 
	{
...
VL53L1_Error getDistance_Simply(VL53L1_Dev_t* pDev,int32_t *distance_out)
{
	VL53L1_Error status = VL53L1_ERROR_NONE;
	uint8_t buffer[2] = {0};
	uint16_t i2c_index = VL53L1_RESULT__FINAL_CROSSTALK_CORRECTED_RANGE_MM_SD0;
	status =VL53L1_ReadMulti(pDev, i2c_index, buffer, 2);
	uint16_t raw = VL53L1_i2c_decode_uint16_t(2, buffer);
	int32_t range_mm = (int32_t)raw;
	int32_t gain = (int32_t)pDev->Data.LLData.gain_cal.standard_ranging_gain_factor;

	range_mm = (range_mm * gain + 0x0400) / 0x0800;
	*distance_out = range_mm;
	//status = VL53L1_ClearInterruptAndStartNext(pDev);
    status = VL53L1_init_and_start_range(pDev,pDev->Data.LLData.measurement_mode,VL53L1_DEVICECONFIGLEVEL_SYSTEM_CONTROL);
	//status = VL53L1_ClearInterruptAndStartMeasurement(pDev);
	return status;
}
